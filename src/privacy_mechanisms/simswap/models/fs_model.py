...
from src.privacy_mechanisms.simswap.models.arcface_models import IRBlock, ResNet
...
class fsModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.crop_size == 224:
            from .fs_networks import Discriminator, Generator_Adain_Upsample
        elif opt.crop_size == 512:
            from .fs_networks_512 import Discriminator, Generator_Adain_Upsample
        ...
        self.netArc.load_state_dict(
            torch.load(f"{SIMSWAP_PATH_HEAD}//arcface_model//netArc.pt")
        )
            ...
            for i in range(num_D):
                for j in range(0, len(fea_fake[i]) - 1):
                    loss_G_GAN_Feat += (
                        D_weights
                        * feat_weights
                        * self.criterionFeat(fea_fake[i][j], fea_real[i][j].detach())
